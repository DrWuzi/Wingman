...
export default function Home({navigation}: Readonly<Props>): React.JSX.Element {
    ...
    React.useEffect(() => {
        async function fetchData() {
            try {
                ...
                const ownerItemsResponse = await api.getUserApi().getUserOwnedItems(activeUser);
                const ownedItems = ownerItemsResponse.value;
                const allAgents: Record<string, IAgent> = (await new AgentsByUuidEndpoint().query(gameContentClient)).unwrap();
    
                if (!ownedItems) {
                    logError('Home: No owned agents: ' + ownerItemsResponse.errorMessage);
                    return;
                }
    
                setAgents(ownedItems.agents.map((uuid: string) => allAgents[uuid]));
    ...
    return (
        <View style={{height: '100%'}}>
            <Text>Home</Text>
                {maps ? <FlatList key={cumColumns} data={maps} numColumns={cumColumns} keyExtractor={item => item.displayName} renderItem={({item}) => 
                        ...
                        <ImageBackground source={{uri: item.splash}} style={styles.boxImage}>
                            ...
                            >
                                ...
                                <View style={styles.agentImagesContainer}>
                                    {agentBlueBg && <ImageBackground source={agentBlueBg} style={styles.agentImageContainer}>
                                        <Image source={{uri: agentIconUrl}} style={styles.agentImage} />
                                    </ImageBackground>}
                                    {agentRedBg && <ImageBackground source={agentRedBg} style={styles.agentImageContainer}>
                                        <Image source={{uri: agentIconUrl}} style={styles.agentImage} />
                                    </ImageBackground>}
                                </View>
                            </LinearGradient>
